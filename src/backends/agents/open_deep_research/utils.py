...
@tool(description="DuckDuckGo web search with simple page fetching and summarization")
async def duckduckgo_search(
    ...
    Args:
        queries: List of search queries to execute
        max_results: Maximum number of results per query
        include_raw_content: Whether to fetch and include page content
        config: Runtime configuration

    Returns:
        Formatted string of deduplicated search results with optional summaries
    """
    # Step 1: Execute searches synchronously via DDGS in thread executor
    ...
    for query in queries:
        results = await loop.run_in_executor(None, _ddg_text, query)
    ...
    # Step 3: Optionally fetch raw page content
    if include_raw_content and unique_results:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            tasks = {
                url: asyncio.create_task(fetch(url))
                for url in list(unique_results.keys())
            }
            fetched = await asyncio.gather(*tasks.values())
            for (url, text_content) in zip(tasks.keys(), fetched):
                if text_content:
                    unique_results[url]["raw_content"] = text_content

    # Step 4: Summarize if raw content available using summarization model
    configurable = Configuration.from_runnable_config(config)
    max_char_to_include = configurable.max_content_length
    if unique_results:
        model_api_key = get_api_key_for_model(configurable.summarization_model, config)
        ...
        async def summarize_or_passthrough(result: dict):
        ...
        summaries = await asyncio.gather(
            *[summarize_or_passthrough(r) for r in unique_results.values()]
        )
        for r, s in zip(unique_results.values(), summaries):
            r["summary"] = s
    ...
    formatted_output = "Search results: \n\n"
    for i, (url, r) in enumerate(unique_results.items(), start=1):
...
def get_model_provider_for_model(model_name: str) -> Optional[str]:
    """Infer LangChain model_provider from a model string prefix.

    Examples:
        "openai:gpt-4.1" -> "openai"
        "anthropic:claude-3" -> "anthropic"
        "google:gemini-1.5" or "gemini:1.5" -> "google"
    ...
    if name.startswith("anthropic:"):
        return "anthropic"
    if name.startswith("google:") or name.startswith("gemini:"):
        return "google"
