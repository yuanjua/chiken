...
async def set_provider_base_url(request: SetProviderBaseUrlRequest):
    """Set the base URL for a provider (updates base_url field)."""
    await ManagerSingleton.update_user_config(base_url=request.base_url)
    return {"status": "success", "provider": request.provider, "base_url": request.base_url}
...
@router.post("/models/ollama", response_model=OllamaModelResponse)
async def get_ollama_model_list(request: OllamaModelListRequest):
    """Get list of available Ollama models."""
    model_data = await LLMService.get_ollama_model_list(base_url=request.base_url)
