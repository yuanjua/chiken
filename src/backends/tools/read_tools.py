    ...
    try:
        ...
        result = {
            ...
            "content_sample": {
                ...
                "avg_content_length": sum(len(doc.get("content", "")) for doc in sample_docs) // len(sample_docs) if sample_docs else 0
            }
        }
...
async def get_document_by_id(document_id: str, collection_name: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        reconstructed_doc = await RAGService.reconstruct_document_from_chunks(
            document_id=document_id,
            knowledge_base_names=search_kb_names
        )
...
async def query_documents_with_context(
    ...
    include_metadata_filter: Optional[List[str]] = None,
    where: Optional[Dict[str, Any]] = None,
    where_document: Optional[Dict[str, str]] = None
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        for result in base_results:
            ...
            if include_metadata_filter and result.get("metadata"):
                filtered_metadata = {
                    ...
                    if key in include_metadata_filter
                }
        ...
        logger.info(f"Enhanced query completed: {len(enhanced_results)} results with avg similarity {result_summary['avg_similarity']}%")
