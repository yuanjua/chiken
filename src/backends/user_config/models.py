...
class UserConfig(BaseModel):
    ...
    base_url: Optional[str] = Field(
        ...
        description="Optional base URL for local providers (e.g., Ollama, custom OpenAI-compatible servers)"
    )
    
    # === Generation Parameters ===
    temperature: float = Field(default=0.7, ge=0.0, le=2.0, title="Temperature")
    max_tokens: Optional[int] = Field(default=32 * 1024, title="Max Tokens")
    ...
    embed_provider: str = Field(default="ollama", title="Embedding Provider", description="Provider for embedding model (e.g., 'ollama', 'openai', 'anthropic')")
    ...
    config_id: Optional[str] = Field(default=None, title="Configuration ID")
    ...
    use_custom_endpoints: bool = Field(default=False, title="Use Custom Endpoints", description="Use custom endpoints stored in database instead of config")
    
    # === PDF Parser Configuration ===
    pdf_parser_type: str = Field(default="local", title="PDF Parser Type")
    pdf_parser_url: Optional[str] = Field(default=None, title="PDF Parser URL")
    
    # === Web Search Configuration ===
    search_engine: str = Field(default="searxng", title="Search Engine")
    search_endpoint: Optional[str] = Field(default="http://localhost:8888", title="Search Endpoint")
    search_api_key: Optional[str] = Field(default=None, title="Search API Key")
    
    # === MCP Configuration ===
    mcp_transport: str = Field(default="stdio", title="MCP Transport Type")
    mcp_port: Optional[int] = Field(default=8000, title="MCP Port")
    
    # === Knowledge Base Configuration ===
    active_knowledge_base_ids: Optional[List[str]] = Field(
    ...
    # === Timestamps ===
    created_at: Optional[str] = Field(default=None, title="Created At")
    updated_at: Optional[str] = Field(default=None, title="Updated At")
    ...
    def get_llm_config(self) -> Dict[str, Any]:
        ...
        # Determine if base_url should be included based on provider type
        provider_type = self.provider_type
        include_base_url = provider_type in ["ollama"] or (
            provider_type == "unknown" and self.base_url
        )
        ...
        if include_base_url and self.base_url:
