...
class ZoteroService:
    ...
    def __init__(self, library_id: str = "0", library_type: str = "user", api_key: str = None, is_local: bool = True):
    ...
    async def is_locally_connected(self) -> Dict[str, Any]:
        ...
        return {'connected': bool(self.zot), 'error': None if self.zot else 'Zotero client not initialized'}
                ...
                async with aiohttp.ClientSession() as session:
                    ...
                    async with session.get(url) as response:
                        if response.status != 200:
                            ...
                            raise HTTPException(status_code=502, detail=f"Error getting item: Zotero API returned {response.status}")
    ...
    async def get_pdf_path(self, item_key: str) -> str:
        ...
        try:
            ...
            if 'links' in item and 'enclosure' in item['links']:
                ...
                raise HTTPException(status_code=502, detail=f"Error getting PDF path: No enclosure link found")
        except Exception as e:
    ...
    async def get_pdf_bytes(self, item_key: str) -> bytes:
        ...
        raise HTTPException(status_code=502, detail=f"Error getting PDF bytes: both zot.item() and get_pdf_path() failed")
    ...
    async def get_random_pdf_key(self) -> Optional[str]:
        """
