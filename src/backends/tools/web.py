    ...
    try:
        async with session.get(url, timeout=DEFAULT_TIMEOUT, headers=DEFAULT_HEADERS) as resp:
            ...
            for entry in feed.entries:
                ...
                venue = getattr(entry, 'arxiv_primary_category', {}).get('term', '') if hasattr(entry, 'arxiv_primary_category') else ''
                results.append({
                    "title": entry.title,
                    "url": entry.link,
...
async def crossref_search(query: str, session: aiohttp.ClientSession, limit: int = 3) -> Dict:
    # No changes needed here, it's working well.
    mailto = os.getenv("ACADEMIC_MAILTO", "devnull@example.com")
...
async def openalex_search(query: str, session: aiohttp.ClientSession, limit: int = 3) -> Dict:
    ...
    url = f"https://api.openalex.org/works?filter=title_and_abstract.search:{quote_plus(query)}"
    url += f"&per-page={int(limit)}"
    if mailto:
...
async def meta_search_tool(
    ...
    *,
    ...
    Args:
      ...
      per_source_limit: Max results to request from each provider (default 3).
