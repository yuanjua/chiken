...
export function useProviderConnection(
  ...
  const testConnection = useCallback(async (provider: string, baseUrl?: string, apiKey?: string) => {
    ...
    try {
      if (provider === "ollama") {
        const response = await getOllamaModelList(baseUrl || "", currentController.signal);
  ...
  const debouncedTestConnection = useCallback((provider: string, baseUrl?: string, apiKey?: string) => {
    ...
    debounceTimeoutRef.current = setTimeout(() => {
      testConnection(provider, baseUrl, apiKey);
    }, debounceDelay);
  }, [testConnection]);
  ...
  useEffect(() => {
    ...
    if (currentProvider === "ollama" && selectedModel.baseUrl) {
      debouncedTestConnection(currentProvider, selectedModel.baseUrl, selectedModel.apiKey);
  ...
  }, [selectedModel.provider, selectedModel.baseUrl, selectedModel.apiKey, debouncedTestConnection]);
