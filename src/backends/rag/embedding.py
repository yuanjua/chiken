...
async def get_embedding_function(model=None):
    ...
    # Determine provider
    provider = extract_provider_from_model(embed_model) if is_litellm_format(embed_model) else (user_config.embed_provider or "ollama")
    ...
    PROVIDERS_REQUIRING_API_KEYS = ("openai", "anthropic", "cohere", "azure", "openrouter", "replicate")
    if provider in PROVIDERS_REQUIRING_API_KEYS and not api_key:
        raise HTTPException(status_code=400, detail=f"Missing API key for embedding provider '{provider}'. Please set it in Environment Variables.")
