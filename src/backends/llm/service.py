...
class LLMService:
    ...
    async def get_llm_config(config: UserConfig) -> Dict[str, Any]:
        ...
        try:
            ...
            if provider_type == "ollama" and config.base_url:
                try:
                    ...
                    async with aiohttp.ClientSession(timeout=timeout) as session:
                        async with session.get(f"{config.base_url}/api/tags") as response:
            ...
            return {
                ...
                "base_url": config.base_url if provider_type == "ollama" else None,
                "api_key": config.api_key if provider_type in ["openai", "anthropic"] else None,
    ...
    async def get_model_completion_suggestions(provider: str, partial_model: str = "", base_url: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        """
        try:
            # For Ollama, use the actual instance if base_url is provided
            if provider == "ollama" and base_url:
                try:
                    # Get models from the specific Ollama instance
                    model_data = await LLMService.get_ollama_model_list(base_url)
    ...
    @staticmethod
    async def get_ollama_model_list(base_url: str) -> Dict[str, Any]:
