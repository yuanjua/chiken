...
class DocumentModel(BaseModel):
    ...
    source: str = Field(
        ...,
        description="A unique identifier for the document source, e.g., a file name, Zotero key, or URL.",
    )
    metadata: dict[str, Any] | None = Field(None, description="Optional metadata like page number, author, etc.")
...
class QueryRequest(BaseModel):
    ...
    knowledge_base_names: list[str] = Field(
        ...,
        json_schema_extra={"example": ["product_docs_v2"]},
...
class QueryResultItem(BaseModel):
    ...
    distance: float
    knowledge_base_name: str | None = Field(None, description="Source knowledge base")
...
class CreateKnowledgeBaseRequest(BaseModel):
    ...
    chunk_size: int | None = Field(1000, description="Chunk size for text splitting")
    chunk_overlap: int | None = Field(200, description="Overlap between chunks")
    embed_model: str | None = Field(None, description="Embedding model to use for this KB")
    enable_reference_filtering: bool | None = Field(True, description="Enable filtering of references and citations")
...
async def add_documents(request: AddDocumentsRequest):
    ...
    try:
        result = await RAGService.add_documents_to_knowledge_base(
            documents=request.documents, knowledge_base_name=request.knowledge_base_name
        )
        return result
...
async def query_documents_endpoint(request: QueryRequest):
    ...
    formatted_results = [QueryResultItem(**item) for item in result]
...
async def zotero_bulk_add_endpoint(request: ZoteroBulkAddRequest):
    ...
    return await RAGService.zotero_bulk_add_to_knowledge_base(
        zotero_keys=request.zotero_keys, knowledge_base_name=request.knowledge_base_name
    )
...
async def get_active_knowledge_bases():
    ...
    return ActiveKnowledgeBasesResponse(
        active_knowledge_bases=active_ids, message=f"Current active knowledge bases: {active_ids}"
    )
...
async def set_active_knowledge_bases(request: ActiveKnowledgeBasesRequest):
    ...
    if success:
        ...
        raise HTTPException(status_code=500, detail="Failed to set active knowledge bases")
...
async def upload_pdf_to_knowledge_base(file: UploadFile = File(...), knowledge_base_name: str = Form(...)):
    """
    ...
    try:
        ...
        result = await RAGService.process_uploaded_pdf(
            pdf_bytes=pdf_bytes, filename=file.filename, knowledge_base_name=knowledge_base_name
        )
...
async def extract_text_from_pdf(file: UploadFile = File(...)):
    ...
    try:
        ...
        return {"success": True, "text": full_text.strip(), "title": filename, "filename": filename}
        ...
        if "MissingDependencyError" in error_msg or "Pandoc" in error_msg:
            detail = (
                f"Missing system dependency: {str(e)}. "
            ...
            detail = (
                f"Remote PDF parser failed: {str(e)}. "
...
async def upload_document_unified(
    file: UploadFile = File(...), knowledge_base_name: str = Form(default="uploaded-documents")
):
    """
    ...
    try:
        ...
        result = await RAGService.process_uploaded_pdf(
            pdf_bytes=pdf_bytes, filename=filename, knowledge_base_name=knowledge_base_name
        )
        ...
        return {"success": False, "error": e.detail, "detail": f"Upload failed: {e.detail}"}
    except Exception as e:
