...
from .utils import remove_references
...
class RAGDB:
    ...
    def _sync_add_chunks_to_collection(self, chunks: List[Document], collection_name: str):
        ...
        try:
            ...
            ids_to_add = [f"{chunk.metadata.get('source', 'unknown_source')}_{i}_{timestamp}_{hash(chunk.page_content[:100]) % 1000000}" for i, chunk in enumerate(valid_chunks)]
            
            logger.debug(f"Prepared {len(documents_to_add)} documents, {len(metadatas_to_add)} metadata objects, {len(ids_to_add)} IDs")
    ...
    async def add_chunks_to_collection(self, chunks: List[Document], collection_name: str):
        ...
        try:
            ...
            chunks_added = await loop.run_in_executor(
                _executor,
                self._sync_add_chunks_to_collection,
                chunks,
                collection_name
            )
            return chunks_added
    ...
    def query(self, query_text: str, collection_name: str, k: int = 10, keys: Union[str, List[str]] = None, where: Optional[Dict[str, Any]] = None, where_document: Optional[Dict[str, str]] = None):
        """
    ...
    def get_documents_by_metadata(self, collection_name: str, where: Dict[str, Any], include_content: bool = True) -> List[Dict[str, Any]]:
        """
    ...
    async def find_document_by_source_or_key(self, source: str, key: str = None, active_kb_ids: List[str] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if key:
                try:
                    uploaded_collection = self.client.get_collection("uploaded_files", embedding_function=self.embeddings)
...
async def add_documents_to_kb(documents: List[Dict[str, Any]], knowledge_base_name: str) -> List[str]:
    ...
    def _split_documents():
        ...
        for doc_item in documents:
            ...
            if not content or not source:
                logger.warning(f"Skipping document due to missing content or source: source='{source}', has_content={bool(content)}")
                continue
            
            # Create base metadata and merge optional data from the request
            base_metadata = {
                "source": source,
                "knowledge_base": actual_kb_name
            }
    ...
    try:
        ...
        chunks_added = await rag_db.add_chunks_to_collection(chunks=all_chunks, collection_name=kb_id)  # Use resolved UUID
