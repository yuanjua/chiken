...
class DatabaseManager:
    ...
    async def initialize(self):
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            # Create provider API keys table for storing provider configurations
            await db.execute("""
                CREATE TABLE IF NOT EXISTS provider_api_keys (
                    provider TEXT PRIMARY KEY,
                    api_key TEXT,
                    base_url TEXT,
                    use_custom_endpoint BOOLEAN DEFAULT 0,
                    created_at TEXT NOT NULL,
                    updated_at TEXT NOT NULL
                )
            """)
            
            # Create index for provider_api_keys
            await db.execute("""
                CREATE INDEX IF NOT EXISTS idx_provider_api_keys_provider 
                ON provider_api_keys(provider)
            """)
            
            # LangGraph checkpointer tables will be created automatically by AsyncSqliteSaver
