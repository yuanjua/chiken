...
import aiohttp
import pytest
from src.backends.rag.custom_ollama_embedding import (
    CustomOllamaEmbeddingFunction,
)

# Import the modules we're testing
from src.backends.rag.db import RAGDB, add_documents_to_kb
from src.backends.rag.embedding import LiteLLMEmbeddingFunction, get_embedding_function
...
class TestCustomOllamaEmbeddingFunction:
    ...
    async def test_fallback_to_secondary_url(self, embedding_function):
        ...
        primary_error = aiohttp.ClientResponseError(request_info=Mock(), history=(), status=502, message="Bad Gateway")

        fallback_response = {"embeddings": [[0.6, 0.7, 0.8, 0.9, 1.0]]}
    ...
    async def test_retry_logic_with_exponential_backoff(self, embedding_function):
        ...
        with patch("aiohttp.ClientSession.post") as mock_post, patch("asyncio.sleep") as mock_sleep:
                ...
                AsyncMock(
                    return_value=AsyncMock(
                        __aenter__=AsyncMock(
                            return_value=Mock(status=200, json=AsyncMock(return_value={"embeddings": [[1, 2, 3]]}))
                        ),
                        __aexit__=AsyncMock(return_value=None),
                    )
                ),
    ...
    def test_synchronous_interface(self, embedding_function):
        ...
        with (
            patch.object(embedding_function, "_async_get_embeddings") as mock_async,
...
class TestRAGService:
    ...
    async def test_set_active_knowledge_bases_with_validation(self):
        ...
        mock_db_manager.list_knowledge_bases = AsyncMock(return_value=[{"id": "kb1"}, {"id": "kb2"}, {"id": "kb3"}])

        with (
            patch("src.backends.rag.service.get_database_manager", return_value=mock_db_manager),
    ...
    async def test_query_documents_with_multiple_kbs(self):
        ...
        mock_db_manager.get_knowledge_base_by_id = AsyncMock(
            return_value={"id": "test_kb", "embed_model": "test-embed-model"}
        )
        ...
        with (
            ...
            result = await RAGService.query_documents(query_text="test query", knowledge_base_names=["kb1", "kb2"], k=5)
...
class TestEmbeddingFunction:
    ...
    async def test_get_embedding_function_ollama_custom(self):
        ...
        with (
            patch(
                "src.backends.rag.embedding.ManagerSingleton.get_user_config",
                return_value=mock_user_config,
            ),
            patch(
                "src.backends.rag.embedding.get_custom_ollama_embedding_function",
                return_value=mock_custom_function,
            ) as mock_get_custom,
        ):
    ...
    async def test_get_embedding_function_fallback_to_litellm(self):
        ...
        with (
            ...
            patch(
                "src.backends.rag.embedding.LiteLLMEmbeddingFunction",
                return_value=mock_litellm_function,
            ),
            patch("src.backends.rag.embedding.get_provider_api_key", return_value="test_api_key"),
        ):
            result = await get_embedding_function()
...
async def test_add_documents_to_kb_integration():
    ...
    mock_db_manager.get_knowledge_base_by_id = AsyncMock(return_value={"embed_model": "test-model"})
    ...
    with (
        patch("src.backends.rag.db.get_database_manager", return_value=mock_db_manager),
