...
def get_parser_from_config(config: UserConfig = None):
    ...
    if config.pdf_parser_type == "remote":
        return RemoteParser(config.pdf_parser_url or "http://127.0.0.1:24008")
    else:
        return LocalParser()
...
async def extract_full_text_from_bytes(
    ...
    Args:
        ...
        use_remote: If True, use remote parser; if False, use Kreuzberg parser (local)
        remote_server_url: URL for remote parser server (legacy)
        remote_options: Options for remote parser processing (legacy)
    
    Returns:
        Extracted text content (plain text for Kreuzberg, markdown for remote parser)
    """
    ...
    try:
        if is_remote:
            ...
            if not isinstance(selected_parser, RemoteParser):
                selected_parser = RemoteParser(remote_server_url or "http://127.0.0.1:24008")
            
            # Remote parser returns markdown
            if not file_hash:
            ...
            # Process with remote parser using hash as key
...
async def extract_full_text_from_bytes_with_config(pdf_bytes: bytes, filename: str = None, config_id: str = "default") -> str:
...
class RemoteParserError(Exception):
    """Custom exception for remote parser errors with better categorization."""
...
class RemoteParser:
    """Generic remote parser for servers implementing /predict and /download endpoints."""
    ...
    async def process_pdf(
        ...
        Args:
            ...
            options: Optional processing options for the remote parser
            
        Returns:
            Markdown content as string
            
        Raises:
            RemoteParserError: With specific error type and message for frontend handling
        """
        # Validate PDF bytes first
        if not pdf_bytes or len(pdf_bytes) < 10:
            raise RemoteParserError(
                "Invalid or empty PDF bytes provided",
        ...
        except RemoteParserError:
            # Re-raise our custom errors as-is
            raise
        except (aiohttp.ClientError, asyncio.TimeoutError) as e:
            logger.error(f"Network error with remote parser at {self.server_url}: {str(e)}")
            raise RemoteParserError(
                f"Cannot connect to remote parser server at {self.server_url}. "
                f"Please check if the server is running and accessible. Error: {str(e)}",
                error_type="connection_error",
                status_code=502
            )
        except Exception as e:
            logger.error(f"Unexpected error processing PDF with remote parser: {str(e)}")
            raise RemoteParserError(
                f"Remote parser encountered an unexpected error: {str(e)}",
        ...
        try:
            ...
            async with session.post(f"{self.server_url}/predict", json=payload) as response:
                if response.status == 200:
                    ...
                    logger.info(f"Remote parser successfully processed PDF")
                    return result
                else:
                    error_text = await response.text(encoding='utf-8')
                    logger.error(f"Remote parser /predict failed: {response.status} - {error_text}")
                    
                    # Parse error message for better user feedback
                    if "Data format error" in error_text:
                        raise RemoteParserError(
                            "The PDF file format is corrupted or not supported by the remote parser. "
                        ...
                        raise RemoteParserError(
                            "Remote parser server configuration error. "
                        ...
                        raise RemoteParserError(
                            f"Remote parser server returned error: {error_text}. "
            ...
            raise RemoteParserError(
                f"Failed to connect to remote parser at {self.server_url}: {str(e)}",
                error_type="connection_error",
                status_code=502
            )
    
    def _extract_file_key(self, prediction_result: Dict[str, str], original_file_key: Optional[str]) -> str:
        ...
        if 'markdown_route' in prediction_result:
            ...
            if len(route_parts) >= 3:
                ...
                raise RemoteParserError(
                    f"Remote parser returned invalid response format. Route: {prediction_result['markdown_route']}",
            ...
            raise RemoteParserError(
                "Remote parser did not return a file key for retrieving results",
    ...
    async def _call_download(self, file_key: str) -> str:
        ...
        try:
            ...
            # All attempts failed
            raise RemoteParserError(
                f"Failed to retrieve processed content from remote parser after {len(delays)} attempts. "
