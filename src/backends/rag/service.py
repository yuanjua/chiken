...
from .db import add_documents_to_kb, RAGDB, get_embeddings_for_kb
...
class RAGService:
    ...
    async def _initialize_active_knowledge_bases():
        ...
        try:
            ...
            if (user_config and 
                hasattr(user_config, 'active_knowledge_base_ids') and 
                user_config.active_knowledge_base_ids):
    ...
    @staticmethod
    async def create_knowledge_base(name: str, description: str = None, chunk_size: int = None, chunk_overlap: int = None, embed_model: str | None = None, enable_reference_filtering: bool = True) -> Dict[str, Any]:
        """
    ...
    async def get_knowledge_base_info(name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            doc_count = 0
            try:
                collection = rag_db.client.get_collection(
                    name=kb_info["id"], 
                    embedding_function=rag_db.embeddings
                )
    ...
    async def add_documents_to_knowledge_base(
        ...
        if not kb_id:
            raise HTTPException(
                status_code=404,
                detail=f"Knowledge base '{knowledge_base_name}' not found"
            )
        ...
        try:
            ...
            result = await add_documents_to_kb(
    ...
    async def _parse_pdf_bytes(pdf_bytes: bytes, filename: str) -> str:
        ...
        try:
            ...
            error_msg = str(e)
            if "remote parser" in error_msg.lower():
                raise ValueError(f"Remote parser failed: {error_msg}. Try switching to Kreuzberg parser in settings.")
            raise ValueError(f"PDF parsing failed: {error_msg}")

    @staticmethod
    async def _process_single_zotero_item(item_key: str, kb_id: str, completed_count: int, total_count: int, progress_callback=None) -> Dict[str, Any]:
        ...
        try:
            ...
            if result["status"] == "completed":
                logger.info(f"[{completed_count}/{total_count}] ✓ Added to DB {item_key}: {result.get('title', 'Unknown')} ({result.get('chunks_added', 0)} chunks)")
                if progress_callback:
    ...
    @staticmethod
    async def zotero_bulk_add_to_knowledge_base(
    ...
    async def process_uploaded_pdf(
        ...
        try:
            ...
            if result["status"] == "completed":
                ...
                raise HTTPException(
                    status_code=500,
                    detail=result.get("error", f"Failed to process {filename}")
                )
    ...
    @staticmethod
    async def get_uploaded_file_by_key(key: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if query:
                ...
                documents = [
                    ...
                    if not knowledge_base_id or knowledge_base_id in RAGService._parse_kb_refs(
                        results["metadatas"][0][i].get("knowledge_base_refs", "")
                    )
                ]
            else:
                results = collection.get(
                ...
                documents = [
                    ...
                    if not knowledge_base_id or knowledge_base_id in RAGService._parse_kb_refs(
                        results["metadatas"][i].get("knowledge_base_refs", "")
                    )
                ]
    ...
    @staticmethod
    async def get_document_by_zotero_key(zotero_key: str) -> Optional[Dict[str, Any]]:
        """
    ...
    async def get_documents_by_zotero_keys(zotero_keys: List[str]) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            ...
            results = collection.get(
                where={"zotero_key": {"$in": zotero_keys}},
                include=["documents", "metadatas"]
            )
    ...
    async def cleanup_uploaded_files_for_kb(knowledge_base_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                return {"cleaned_count": 0, "deleted_count": 0, "error": "uploaded_files collection not found"}
            ...
            logger.info(f"Cleaned up uploaded_files for KB {knowledge_base_id}: {cleaned_count} cleaned, {deleted_count} deleted")
    ...
    async def migrate_uploaded_files_to_kb_refs() -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                return {"migrated_count": 0, "error": "uploaded_files collection not found", "success": False}
    ...
    async def clear_uploaded_files_collection() -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                return {"success": True, "message": "uploaded_files collection was already empty or didn't exist"}
    ...
    @staticmethod
    async def _centralized_document_exists_check(content_hash: str, kb_id: str) -> Optional[Dict[str, Any]]:
        """
    ...
    async def _process_and_add_document(
        ...
        try:
            ...
            if chunks_added == 0:
                ...
                return {"key": source, "status": "failed", "error": "No chunks were added to knowledge base"}
            ...
            try:
                ...
                logger.warning(f"Continuing despite uploaded_files error - KB addition was successful")
        ...
        try:
            ...
            for source_info in unique_sources:
                ...
                documents_list.append({
                    "source": source,
                    "title": title,
                    "key": content_hash if content_hash else source  # Use content hash if available, fallback to source
                })
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Failed to get documents from knowledge base: {str(e)}"
            )

    @staticmethod
    async def reconstruct_document_from_chunks(document_id: str, knowledge_base_names: Optional[List[str]] = None) -> Optional[Dict[str, Any]]:
        """
            ...
            for i, chunk in enumerate(unique_chunks):
                ...
                if i == 0:
                    base_metadata = {k: v for k, v in metadata.items() 
                                   if k not in ["chunk_id", "chunk_index", "chunk_size"]}
